...
func GetDeviceGrpcClient(ctx context.Context, r client.Reader, obj *v1alpha1.Device) (*grpc.ClientConn, error) {
	...
	if obj.Spec.TLS != nil {
		ca, err := c.Secret(ctx, obj.Spec.TLS.CA)
		if err != nil {
			return nil, err
		}
		...
		if certPool := x509.NewCertPool(); certPool.AppendCertsFromPEM(ca) {
			...
			if obj.Spec.TLS.Certificate != nil {
				cert, err := c.Certificate(ctx, obj.Spec.TLS.Certificate.SecretRef)
				if err != nil {
					return nil, err
				}
	...
	if obj.Spec.SecretRef != nil {
		user, pass, err := c.BasicAuth(ctx, obj.Spec.SecretRef)
		if err != nil {
			return nil, err
		}
	...
	return grpc.NewClient(obj.Spec.Endpoint, opts...)
}
