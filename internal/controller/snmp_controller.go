...
func (r *SNMPReconciler) reconcile(ctx context.Context, s *snmpScope) (_ ctrl.Result, reterr error) {
	...
	if !controllerutil.HasControllerReference(s.SNMP) {
		if err := controllerutil.SetOwnerReference(s.Device, s.SNMP, r.Scheme); err != nil {
			return ctrl.Result{}, err
		}
	}
	...
	for _, c := range res.Conditions {
		meta.SetStatusCondition(&s.SNMP.Status.Conditions, c)
	}
	if err != nil {
		return ctrl.Result{}, err
	}

	meta.SetStatusCondition(&s.SNMP.Status.Conditions, metav1.Condition{
		Type:               v1alpha1.ReadyCondition,
		Status:             metav1.ConditionTrue,
		Reason:             v1alpha1.ReadyReason,
		Message:            "SNMP configured successfully",
		ObservedGeneration: s.SNMP.Generation,
	})
