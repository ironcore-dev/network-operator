...
func (r *BannerReconciler) reconcile(ctx context.Context, s *bannerScope) (_ ctrl.Result, reterr error) {
	...
	if !controllerutil.HasControllerReference(s.Banner) {
		if err := controllerutil.SetOwnerReference(s.Device, s.Banner, r.Scheme); err != nil {
			return ctrl.Result{}, err
		}
	}
	...
	meta.SetStatusCondition(&s.Banner.Status.Conditions, metav1.Condition{
		Type:               v1alpha1.ReadyCondition,
		Status:             metav1.ConditionTrue,
		Reason:             v1alpha1.ReadyReason,
		Message:            "Banner configured successfully",
		ObservedGeneration: s.Banner.Generation,
	})
