...
func (r *ISISReconciler) reconcile(ctx context.Context, s *isisScope) (_ ctrl.Result, reterr error) {
	...
	if !controllerutil.HasControllerReference(s.ISIS) {
		if err := controllerutil.SetOwnerReference(s.Device, s.ISIS, r.Scheme); err != nil {
			return ctrl.Result{}, err
		}
	}
	...
	meta.SetStatusCondition(&s.ISIS.Status.Conditions, metav1.Condition{
		Type:               v1alpha1.ReadyCondition,
		Status:             metav1.ConditionTrue,
		Reason:             v1alpha1.ReadyReason,
		Message:            "ISIS configured successfully",
		ObservedGeneration: s.ISIS.Generation,
	})
