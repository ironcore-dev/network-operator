...
func (r *AccessControlListReconciler) reconcile(ctx context.Context, s *aclScope) (_ ctrl.Result, reterr error) {
	...
	if !controllerutil.HasControllerReference(s.ACL) {
		if err := controllerutil.SetOwnerReference(s.Device, s.ACL, r.Scheme); err != nil {
			return ctrl.Result{}, err
		}
	}

	if err := s.ACL.Validate(); err != nil {
		ctrl.LoggerFrom(ctx).Error(err, "Invalid AccessControlList spec")
		meta.SetStatusCondition(&s.ACL.Status.Conditions, metav1.Condition{
			Type:               v1alpha1.ReadyCondition,
			Status:             metav1.ConditionFalse,
			Reason:             v1alpha1.ValidationFailedReason,
			Message:            fmt.Sprintf("Invalid spec: %v", err),
			ObservedGeneration: s.ACL.Generation,
		})
		return ctrl.Result{}, nil
	}
	...
	meta.SetStatusCondition(&s.ACL.Status.Conditions, metav1.Condition{
		...
		Message:            "AccessControlList configured successfully",
		ObservedGeneration: s.ACL.Generation,
	})
