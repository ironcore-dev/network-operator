...
func (r *ManagementAccessReconciler) reconcile(ctx context.Context, s *managementAccessScope) (_ ctrl.Result, reterr error) {
	...
	if !controllerutil.HasControllerReference(s.ManagementAccess) {
		if err := controllerutil.SetOwnerReference(s.Device, s.ManagementAccess, r.Scheme); err != nil {
			return ctrl.Result{}, err
		}
	}
	...
	meta.SetStatusCondition(&s.ManagementAccess.Status.Conditions, metav1.Condition{
		Type:               v1alpha1.ReadyCondition,
		Status:             metav1.ConditionTrue,
		Reason:             v1alpha1.ReadyReason,
		Message:            "ManagementAccess configured successfully",
		ObservedGeneration: s.ManagementAccess.Generation,
	})
