...
func (r *VTEPReconciler) reconcile(ctx context.Context, s *vtepScope) (reterr error) {
	...
	if s.ProviderConfig != nil {
		if err := r.validateProviderConfigRef(ctx, s); err != nil {
			...
			return fmt.Errorf("provider config reference is not compatible with the device: %w", err)
		}
	}
...
func (r *VTEPReconciler) validateProviderConfigRef(_ context.Context, s *vtepScope) error {
	gv, err := schema.ParseGroupVersion(s.VTEP.Spec.ProviderConfigRef.APIVersion)
	if err != nil {
		return fmt.Errorf("invalid apiVersion %q: %w", s.VTEP.Spec.ProviderConfigRef.APIVersion, err)
	}

	if found := slices.Contains(v1alpha1.VTEPDependencies, schema.GroupVersionKind{
		Group:   gv.Group,
		Version: gv.Version,
		Kind:    s.VTEP.Spec.ProviderConfigRef.Kind,
	}); !found {
		return fmt.Errorf("unsupported provider config ref kind %q for VTEP on the provider", gv)
	}
	return nil
}
