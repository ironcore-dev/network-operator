...
func (r *DHCPRelayReconciler) Reconcile(ctx context.Context, req ctrl.Request) (_ ctrl.Result, reterr error) {
	...
	prov, err := provider.LoadProvider[provider.DHCPRelayProvider](device.Spec.Provider)
	if err != nil {
...
func (r *DHCPRelayReconciler) finalize(ctx context.Context, s *dhcprelayScope) (reterr error) {
	...
	req := provider.DeleteDHCPRelayRequest{
		DHCPRelay: s.DHCPRelay,
	}
	...
	// Skip finalization if another DHCPRelay exists for the same interface on the same device, e.g.,
	// a resource rejected by the controller uniqueness will still be finalized and would delete
	// the configuration of the other resource.
	var list v1alpha1.DHCPRelayList
	if err := r.List(
		ctx, &list,
		client.InNamespace(s.DHCPRelay.Namespace),
		...
		return fmt.Errorf("failed to list DHCPRelays for device: %w", err)
	}
	for i := range list.Items {
		other := &list.Items[i]
		if other.Name != s.DHCPRelay.Name && other.DeletionTimestamp.IsZero() &&
			other.Spec.InterfaceRef != nil && other.Spec.InterfaceRef.Name == s.DHCPRelay.Spec.InterfaceRef.Name {
			return nil
		}
	}
