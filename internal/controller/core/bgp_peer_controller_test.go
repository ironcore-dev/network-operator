	...
	Context("When reconciling a resource", func() {
		...
		It("Should remove an unnumbered BGP peer from the provider after its Interface was deleted", func() {
			...
			Eventually(func(g Gomega) {
				...
				g.Expect(testProvider.BGPPeers.Has("Ethernet1/1")).To(BeTrue())
			}).Should(Succeed())
			...
			Eventually(func(g Gomega) {
				g.Expect(testProvider.BGPPeers.Has("Ethernet1/1")).To(BeFalse(), "Provider should not have the unnumbered BGP peer configured")
			}).Should(Succeed())
		})
