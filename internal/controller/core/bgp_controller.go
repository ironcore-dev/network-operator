...
func (r *BGPReconciler) Reconcile(ctx context.Context, req ctrl.Request) (_ ctrl.Result, reterr error) {
	...
	defer func() {
		if err := r.Locker.ReleaseLock(ctx, device.Name, "bgp-controller"); err != nil {
...
func (r *BGPReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	for _, gvk := range v1alpha1.BGPDependencies {
		obj := &unstructured.Unstructured{}
		obj.SetGroupVersionKind(gvk)

		bldr = bldr.Watches(
			obj,
			handler.EnqueueRequestsFromMapFunc(r.bgpForProviderConfig),
...
// bgpForProviderConfig is a [handler.MapFunc] to be used to enqueue requests for reconciliation
// for a BGP to update when one of its referenced provider configurations gets updated.
func (r *BGPReconciler) bgpForProviderConfig(ctx context.Context, obj client.Object) []reconcile.Request {
...
func (r *BGPReconciler) vrfToBGPs(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	for _, b := range list.Items {
		if b.Spec.VrfRef != nil && b.Spec.VrfRef.Name == vrf.Name {
