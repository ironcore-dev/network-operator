...
func (r *PIMReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager) error {
	...
	for _, gvk := range v1alpha1.PIMDependencies {
		obj := &unstructured.Unstructured{}
		obj.SetGroupVersionKind(gvk)

		bldr = bldr.Watches(
			obj,
			handler.EnqueueRequestsFromMapFunc(r.pimForProviderConfig),
		...
		Watches(
			...
			builder.WithPredicates(predicate.Funcs{
				UpdateFunc: func(e event.UpdateEvent) bool {
					...
					return conditions.IsConfigured(oldInterface) != conditions.IsConfigured(newInterface)
				},
				GenericFunc: func(e event.GenericEvent) bool {
					return false
				},
...
func (r *PIMReconciler) finalize(ctx context.Context, s *pimScope) (reterr error) {
	...
	return s.Provider.DeletePIM(ctx)
}
...
// pimForProviderConfig is a [handler.MapFunc] to be used to enqueue requests for reconciliation
// for a PIM to update when one of its referenced provider configurations gets updated.
func (r *PIMReconciler) pimForProviderConfig(ctx context.Context, obj client.Object) []reconcile.Request {
	log := ctrl.LoggerFrom(ctx, "Object", klog.KObj(obj))

	list := &v1alpha1.PIMList{}
	...
	for _, m := range list.Items {
		if m.Spec.ProviderConfigRef != nil &&
			...
			log.V(2).Info("Enqueuing PIM for reconciliation", "PIM", klog.KObj(&m))
			requests = append(requests, reconcile.Request{
				Name:      m.Name,
