...
func (r *StaticRouteReconciler) Reconcile(ctx context.Context, req ctrl.Request) (_ ctrl.Result, reterr error) {
	...
	prov, err := provider.LoadProvider[provider.StaticRouteProvider](device.Spec.Provider)
	if err != nil {
...
func (r *StaticRouteReconciler) reconcile(ctx context.Context, s *staticRouteScope) (reterr error) {
	...
	intfMap, err := r.reconcileIntf(ctx, s)
	if err != nil {
		return err
	}
...
// reconcileIntf ensures that the referenced Interface exists and belongs to the same device as the StaticRoute.
func (r *StaticRouteReconciler) reconcileIntf(ctx context.Context, s *staticRouteScope) (map[string]*v1alpha1.Interface, error) {
	intfMap := make(map[string]*v1alpha1.Interface)
	for _, nexthop := range s.StaticRoute.Spec.NextHops {
		if nexthop.InterfaceRef == nil {
			continue
		}
		intf := new(v1alpha1.Interface)
		key := client.ObjectKey{
			Name:      nexthop.InterfaceRef.Name,
			Namespace: s.StaticRoute.Namespace,
		}
		...
		if err := r.Get(ctx, key, intf); err != nil {
			...
			conditions.Set(s.StaticRoute, metav1.Condition{
				...
				Status:  metav1.ConditionFalse,
				Reason:  v1alpha1.InterfaceNotFoundReason,
				Message: fmt.Sprintf("failed to get referenced Interface %q: %v", client.ObjectKeyFromObject(intf), err),
			})
			return nil, err
		}
