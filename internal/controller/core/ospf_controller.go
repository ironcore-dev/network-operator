...
func (r *OSPFReconciler) Reconcile(ctx context.Context, req ctrl.Request) (_ ctrl.Result, reterr error) {
	...
	prov, ok := r.Provider().(provider.OSPFProvider)
	if !ok {
	...
	defer func() {
		if err := r.Locker.ReleaseLock(ctx, device.Name, "ospf-controller"); err != nil {
...
func (r *OSPFReconciler) reconcile(ctx context.Context, s *ospfScope) (reterr error) {
	...
	s.OSPF.Labels[v1alpha1.DeviceLabel] = s.Device.Name
	...
	err := s.Provider.EnsureOSPF(ctx, &provider.EnsureOSPFRequest{
		OSPF:           s.OSPF,
	...
	cond = metav1.Condition{
		...
		Message: "OSPF is operational",
	}
	if !status.OperStatus {
		cond.Status = metav1.ConditionFalse
		cond.Reason = v1alpha1.DegradedReason
		cond.Message = "OSPF is not operational"
	}
	conditions.Set(s.OSPF, cond)

	s.OSPF.Status.Neighbors = make([]v1alpha1.OSPFNeighbor, 0, len(status.Neighbors))
	for _, nbr := range status.Neighbors {
		n := v1alpha1.OSPFNeighbor{
			RouterID:       nbr.RouterID,
			Address:        nbr.Address,
			InterfaceRef:   v1alpha1.LocalObjectReference{Name: nbr.Interface.Name},
			AdjacencyState: nbr.AdjacencyState,
		}
		if nbr.Priority > 0 {
	...
	slices.SortFunc(s.OSPF.Status.Neighbors, func(i, j v1alpha1.OSPFNeighbor) int {
		return cmp.Compare(i.InterfaceRef.Name, j.InterfaceRef.Name)
	})

	states := make(map[v1alpha1.OSPFNeighborState]int)
	for _, nbr := range s.OSPF.Status.Neighbors {
		states[nbr.AdjacencyState]++
	}

	var summaries []string
	for _, state := range []v1alpha1.OSPFNeighborState{
		v1alpha1.OSPFNeighborStateFull,
		v1alpha1.OSPFNeighborStateLoading,
		v1alpha1.OSPFNeighborStateExchange,
		v1alpha1.OSPFNeighborStateExStart,
		v1alpha1.OSPFNeighborStateTwoWay,
		v1alpha1.OSPFNeighborStateInit,
		v1alpha1.OSPFNeighborStateAttempt,
		v1alpha1.OSPFNeighborStateDown,
		v1alpha1.OSPFNeighborStateUnknown,
	} {
		if count := states[state]; count > 0 {
			summaries = append(summaries, fmt.Sprintf("%d %s", count, state))
		}
	}
...
func (r *OSPFReconciler) finalize(ctx context.Context, s *ospfScope) (reterr error) {
	...
	return s.Provider.DeleteOSPF(ctx, &provider.DeleteOSPFRequest{
		OSPF:           s.OSPF,
		ProviderConfig: s.ProviderConfig,
	})
}
