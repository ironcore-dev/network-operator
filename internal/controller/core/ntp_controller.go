...
// +kubebuilder:rbac:groups=networking.metal.ironcore.dev,resources=ntp,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=networking.metal.ironcore.dev,resources=ntp/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=networking.metal.ironcore.dev,resources=ntp/finalizers,verbs=update
...
func (r *NTPReconciler) Reconcile(ctx context.Context, req ctrl.Request) (_ ctrl.Result, reterr error) {
	...
	prov, ok := r.Provider().(provider.NTPProvider)
	if !ok {
		if meta.SetStatusCondition(&obj.Status.Conditions, metav1.Condition{
			Type:    v1alpha1.ReadyCondition,
			Status:  metav1.ConditionFalse,
			Reason:  v1alpha1.NotImplementedReason,
			Message: "Provider does not implement provider.NTPProvider",
		}) {
	...
	defer func() {
		if err := r.Locker.ReleaseLock(ctx, device.Name, "ntp-controller"); err != nil {
...
func (r *NTPReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager) error {
	...
	for _, gvk := range v1alpha1.NTPDependencies {
		obj := &unstructured.Unstructured{}
		obj.SetGroupVersionKind(gvk)

		bldr = bldr.Watches(
			obj,
			handler.EnqueueRequestsFromMapFunc(r.ntpForProviderConfig),
...
func (r *NTPReconciler) finalize(ctx context.Context, s *ntpScope) (reterr error) {
	...
	return s.Provider.DeleteNTP(ctx)
}
...
func (r *NTPReconciler) deviceToNTPs(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	for _, i := range list.Items {
		log.V(2).Info("Enqueuing NTP for reconciliation", "NTP", klog.KObj(&i))
...
// ntpForProviderConfig is a [handler.MapFunc] to be used to enqueue requests for reconciliation
// for a NTP to update when one of its referenced provider configurations gets updated.
func (r *NTPReconciler) ntpForProviderConfig(ctx context.Context, obj client.Object) []reconcile.Request {
	log := ctrl.LoggerFrom(ctx, "Object", klog.KObj(obj))

	list := &v1alpha1.NTPList{}
	...
	for _, m := range list.Items {
		if m.Spec.ProviderConfigRef != nil &&
			...
			log.V(2).Info("Enqueuing NTP for reconciliation", "NTP", klog.KObj(&m))
