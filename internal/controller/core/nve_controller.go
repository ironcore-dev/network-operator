...
import (
	...
	nxv1alphav1 "github.com/ironcore-dev/network-operator/api/cisco/nx/v1alpha1"
...
// NVEReconciler reconciles a NVE object
type NVEReconciler struct {
	client.Client
...
// +kubebuilder:rbac:groups=networking.metal.ironcore.dev,resources=nves,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=networking.metal.ironcore.dev,resources=nves/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=networking.metal.ironcore.dev,resources=nves/finalizers,verbs=update
...
func (r *NVEReconciler) Reconcile(ctx context.Context, req ctrl.Request) (_ ctrl.Result, reterr error) {
	log := ctrl.LoggerFrom(ctx)
	log.Info("Reconciling resource")

	obj := new(v1alpha1.NVE)
...
func (r *NVEReconciler) reconcile(ctx context.Context, s *nveScope) (reterr error) {
	if s.NVE.Labels == nil {
	...
	if anycastIf != nil && sourceIf.Name == anycastIf.Name {
		conditions.Set(s.NVE, metav1.Condition{
			Type:    v1alpha1.ConfiguredCondition,
			Status:  metav1.ConditionFalse,
			Reason:  v1alpha1.ErrorReason,
			Message: "SourceInterfaceRef and AnycastSourceInterfaceRef cannot refer to the same interface",
		})
		return reconcile.TerminalError(errors.New("source interface and anycast source interface cannot be the same"))
	}
...
func (r *NVEReconciler) validateInterfaceRef(ctx context.Context, interfaceRef *v1alpha1.LocalObjectReference, s *nveScope) (*v1alpha1.Interface, error) {
	if interfaceRef == nil {
		return nil, nil
	}
	...
	if err := r.Get(ctx, client.ObjectKey{Name: intf.Name, Namespace: intf.Namespace}, intf); err != nil {
		if apierrors.IsNotFound(err) {
			...
			return nil, reconcile.TerminalError(fmt.Errorf("member interface %q not found", s.NVE.Spec.SourceInterfaceRef.Name))
		}
		return nil, reconcile.TerminalError(fmt.Errorf("failed to get member interface %q: %w", s.NVE.Spec.SourceInterfaceRef.Name, err))
	}
...
func (r *NVEReconciler) validateProviderConfigRef(ctx context.Context, s *nveScope) error {
	...
	if err != nil {
		...
		return reconcile.TerminalError(fmt.Errorf("invalid apiVersion %q: %w", s.NVE.Spec.ProviderConfigRef.APIVersion, err))
	}

	if found := slices.Contains(v1alpha1.NVEDependencies, schema.GroupVersionKind{
		Group:   gv.Group,
		Version: gv.Version,
	...
	nveList := &v1alpha1.NVEList{}
	if err := r.List(ctx, nveList, client.InNamespace(s.NVE.Namespace)); err != nil {
		return fmt.Errorf("failed to list NVEs: %w", err)
	}
	for _, v := range nveList.Items {
		if v.Name != s.NVE.Name && v.Spec.DeviceRef.Name == s.NVE.Spec.DeviceRef.Name {
			conditions.Set(s.NVE, metav1.Condition{
				Type:    v1alpha1.ConfiguredCondition,
				Status:  metav1.ConditionFalse,
				Reason:  nxv1alphav1.NVEConfigAlreadyExistsReason,
				Message: fmt.Sprintf("Another NVE (%s) already exists for device %s", v.Name, v.Spec.DeviceRef.Name),
			})
...
func (r *NVEReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager) error {
	...
	for _, gvk := range v1alpha1.NVEDependencies {
...
func (r *NVEReconciler) mapProviderConfigToNVEs(ctx context.Context, obj client.Object) []reconcile.Request {
	log := ctrl.LoggerFrom(ctx, "Object", klog.KObj(obj))

	list := &v1alpha1.NVEList{}
...
func (r *NVEReconciler) mapInterfaceToNVEs(ctx context.Context, obj client.Object) []reconcile.Request {
	intf, ok := obj.(*v1alpha1.Interface)
	if !ok {
		panic(fmt.Sprintf("Expected an Interface but got a %T", obj))
	}
	log := ctrl.LoggerFrom(ctx)
	nves := &v1alpha1.NVEList{}
...
func (r *NVEReconciler) finalize(ctx context.Context, s *nveScope) (reterr error) {
	if err := s.Provider.Connect(ctx, s.Connection); err != nil {
		return fmt.Errorf("failed to connect to provider: %w", err)
	}
	defer func() {
