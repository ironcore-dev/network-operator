...
// bgpPeerInterfaceRefIndexKey is the field index key for all Interface names referenced by
// a BGPPeer, both as unnumbered peer interface and as local (source) address.
...
func (r *BGPPeerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (_ ctrl.Result, reterr error) {
	...
	prov, err := provider.LoadProvider[provider.BGPPeerProvider](device.Spec.Provider)
	if err != nil {
...
func (r *BGPPeerReconciler) reconcile(ctx context.Context, s *bgpPeerScope) (reterr error) {
	...
	if addr := s.BGPPeer.Spec.LocalAddress; addr != nil {
		intf, err := r.reconcileInterfaceRef(ctx, s, addr.InterfaceRef.Name, "source interface")
		if err != nil {
			return err
		}
	...
	if ref := s.BGPPeer.Spec.InterfaceRef; ref != nil {
		intf, err := r.reconcileInterfaceRef(ctx, s, ref.Name, "peer interface")
		if err != nil {
			return err
		}
		peerInterface = intf.Spec.Name
		// Recorded even if configuring the device fails below, so that a partially
		// applied configuration can still be cleaned up once the Interface is gone.
		s.BGPPeer.Status.PeerInterface = peerInterface
	}
...
func (r *BGPPeerReconciler) finalize(ctx context.Context, s *bgpPeerScope) (reterr error) {
	...
	// The interface is the identity of an unnumbered peer on the device. Prefer the name
	// recorded during reconciliation, as the Interface may already have been deleted.
	var peerInterface string
	if ref := s.BGPPeer.Spec.InterfaceRef; ref != nil {
		peerInterface = s.BGPPeer.Status.PeerInterface
		if peerInterface == "" {
			...
			if err := r.Get(ctx, types.NamespacedName{
				...
				// Without the recorded name and the Interface the peer cannot be
				// identified on the device, so we can only proceed with deletion.
				return client.IgnoreNotFound(err)
			}
...
// reconcileInterfaceRef resolves an Interface referenced by the BGPPeer and validates that
// it belongs to the same Device. The description is used in conditions and errors to tell
// the source interface and the unnumbered peer interface apart.
func (r *BGPPeerReconciler) reconcileInterfaceRef(ctx context.Context, s *bgpPeerScope, name, description string) (*v1alpha1.Interface, error) {
	intf := new(v1alpha1.Interface)
	if err := r.Get(ctx, client.ObjectKey{Name: name, Namespace: s.BGPPeer.Namespace}, intf); err != nil {
		if apierrors.IsNotFound(err) {
			conditions.Set(s.BGPPeer, metav1.Condition{
				Type:    v1alpha1.ConfiguredCondition,
				Status:  metav1.ConditionFalse,
				Reason:  v1alpha1.InterfaceNotFoundReason,
				Message: fmt.Sprintf("%s %q not found", description, name),
			})
			return nil, reconcile.TerminalError(fmt.Errorf("%s %q not found", description, name))
		}
		return nil, fmt.Errorf("failed to get %s %q: %w", description, name, err)
	}
	...
	if intf.Spec.DeviceRef.Name != s.Device.Name {
		conditions.Set(s.BGPPeer, metav1.Condition{
			...
			Message: fmt.Sprintf("%s %q does not belong to device %q", description, intf.Name, s.Device.Name),
		})
		return nil, reconcile.TerminalError(fmt.Errorf("%s %q does not belong to device %q", description, intf.Name, s.Device.Name))
	}
...
func (r *BGPPeerReconciler) interfaceToBGPPeers(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	for i := range list.Items {
		log.V(2).Info("Enqueuing BGPPeer for reconciliation", "BGPPeer", klog.KObj(&list.Items[i]))
