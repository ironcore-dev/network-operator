...
// +kubebuilder:rbac:groups=networking.metal.ironcore.dev,resources=syslogs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=networking.metal.ironcore.dev,resources=syslogs/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=networking.metal.ironcore.dev,resources=syslogs/finalizers,verbs=update
...
func (r *SyslogReconciler) Reconcile(ctx context.Context, req ctrl.Request) (_ ctrl.Result, reterr error) {
	...
	prov, err := provider.LoadProvider[provider.SyslogProvider](device.Spec.Provider)
	if err != nil {
	...
	defer func() {
		if err := r.Locker.ReleaseLock(ctx, device.Name, "syslog-controller"); err != nil {
...
func (r *SyslogReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager) error {
	...
	for _, gvk := range v1alpha1.SyslogDependencies {
		obj := &unstructured.Unstructured{}
		obj.SetGroupVersionKind(gvk)

		bldr = bldr.Watches(
			obj,
			handler.EnqueueRequestsFromMapFunc(r.syslogForProviderConfig),
...
func (r *SyslogReconciler) finalize(ctx context.Context, s *syslogScope) (reterr error) {
	...
	return s.Provider.DeleteSyslog(ctx)
}
...
func (r *SyslogReconciler) deviceToSyslogs(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	for _, i := range list.Items {
		log.V(2).Info("Enqueuing Syslog for reconciliation", "Syslog", klog.KObj(&i))
...
// syslogForProviderConfig is a [handler.MapFunc] to be used to enqueue requests for reconciliation
// for a Syslog to update when one of its referenced provider configurations gets updated.
func (r *SyslogReconciler) syslogForProviderConfig(ctx context.Context, obj client.Object) []reconcile.Request {
	log := ctrl.LoggerFrom(ctx, "Object", klog.KObj(obj))

	list := &v1alpha1.SyslogList{}
	...
	for _, m := range list.Items {
		if m.Spec.ProviderConfigRef != nil &&
			...
			log.V(2).Info("Enqueuing Syslog for reconciliation", "Syslog", klog.KObj(&m))
