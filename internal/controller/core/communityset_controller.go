...
func (r *CommunitySetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (_ ctrl.Result, reterr error) {
	...
	prov, err := provider.LoadProvider[provider.CommunitySetProvider](device.Spec.Provider)
	if err != nil {
...
func (r *CommunitySetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager) error {
	...
	for _, gvk := range v1alpha1.CommunitySetDependencies {
		obj := &unstructured.Unstructured{}
		obj.SetGroupVersionKind(gvk)

		bldr = bldr.Watches(
			obj,
			handler.EnqueueRequestsFromMapFunc(r.communitySetsForProviderConfig),
...
// communitySetsForProviderConfig is a [handler.MapFunc] to be used to enqueue requests for reconciliation
// for a CommunitySet to update when one of its referenced provider configurations gets updated.
func (r *CommunitySetReconciler) communitySetsForProviderConfig(ctx context.Context, obj client.Object) []reconcile.Request {
	log := ctrl.LoggerFrom(ctx, "Object", klog.KObj(obj))

	list := &v1alpha1.CommunitySetList{}
	...
	for _, m := range list.Items {
		if m.Spec.ProviderConfigRef != nil &&
			...
			log.V(2).Info("Enqueuing CommunitySet for reconciliation", "CommunitySet", klog.KObj(&m))
