...
func (r *UserReconciler) Reconcile(ctx context.Context, req ctrl.Request) (_ ctrl.Result, reterr error) {
	...
	prov, err := provider.LoadProvider[provider.UserProvider](device.Spec.Provider)
	if err != nil {
	...
	defer func() {
		if err := r.Locker.ReleaseLock(ctx, device.Name, "user-controller"); err != nil {
...
func (r *UserReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager) error {
	...
	for _, gvk := range v1alpha1.UserDependencies {
		obj := &unstructured.Unstructured{}
		obj.SetGroupVersionKind(gvk)

		bldr = bldr.Watches(
			obj,
			handler.EnqueueRequestsFromMapFunc(r.usersForProviderConfig),
...
// usersForProviderConfig is a [handler.MapFunc] to be used to enqueue requests for reconciliation
// for a User to update when one of its referenced provider configurations gets updated.
func (r *UserReconciler) usersForProviderConfig(ctx context.Context, obj client.Object) []reconcile.Request {
