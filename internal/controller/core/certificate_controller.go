...
func (r *CertificateReconciler) Reconcile(ctx context.Context, req ctrl.Request) (_ ctrl.Result, reterr error) {
	...
	prov, err := provider.LoadProvider[provider.CertificateProvider](device.Spec.Provider)
	if err != nil {
...
func (r *CertificateReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager) error {
	...
	for _, gvk := range v1alpha1.CertificateDependencies {
		obj := &unstructured.Unstructured{}
		obj.SetGroupVersionKind(gvk)

		bldr = bldr.Watches(
			obj,
			handler.EnqueueRequestsFromMapFunc(r.certificatesForProviderConfig),
...
// certificatesForProviderConfig is a [handler.MapFunc] to be used to enqueue requests for reconciliation
// for a Certificate to update when one of its referenced provider configurations gets updated.
func (r *CertificateReconciler) certificatesForProviderConfig(ctx context.Context, obj client.Object) []reconcile.Request {
