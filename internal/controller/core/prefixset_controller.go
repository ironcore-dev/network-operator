...
// +kubebuilder:rbac:groups=networking.metal.ironcore.dev,resources=prefixsets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=networking.metal.ironcore.dev,resources=prefixsets/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=networking.metal.ironcore.dev,resources=prefixsets/finalizers,verbs=update
...
func (r *PrefixSetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (_ ctrl.Result, reterr error) {
	...
	prov, ok := r.Provider().(provider.PrefixSetProvider)
	if !ok {
		if meta.SetStatusCondition(&obj.Status.Conditions, metav1.Condition{
			Type:    v1alpha1.ReadyCondition,
			Status:  metav1.ConditionFalse,
			Reason:  v1alpha1.NotImplementedReason,
			Message: "Provider does not implement provider.PrefixSetProvider",
		}) {
	...
	defer func() {
		if err := r.Locker.ReleaseLock(ctx, device.Name, "prefixset-controller"); err != nil {
...
func (r *PrefixSetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager) error {
	...
	for _, gvk := range v1alpha1.PrefixSetDependencies {
		obj := &unstructured.Unstructured{}
		obj.SetGroupVersionKind(gvk)

		bldr = bldr.Watches(
			obj,
			handler.EnqueueRequestsFromMapFunc(r.prefixSetsForProviderConfig),
...
type prefixSetScope struct {
	Device         *v1alpha1.Device
	PrefixSet      *v1alpha1.PrefixSet
	Connection     *deviceutil.Connection
	ProviderConfig *provider.ProviderConfig
	Provider       provider.PrefixSetProvider
}

func (r *PrefixSetReconciler) reconcile(ctx context.Context, s *prefixSetScope) (reterr error) {
	if s.PrefixSet.Labels == nil {
		s.PrefixSet.Labels = make(map[string]string)
	}

	s.PrefixSet.Labels[v1alpha1.DeviceLabel] = s.Device.Name

	defer func() {
		conditions.RecomputeReady(s.PrefixSet)
	}()

	conditions.Set(s.PrefixSet, metav1.Condition{
	...
	// Ensure the PrefixSet is owned by the Device.
	if !controllerutil.HasControllerReference(s.PrefixSet) {
		if err := controllerutil.SetOwnerReference(s.Device, s.PrefixSet, r.Scheme, controllerutil.WithBlockOwnerDeletion(true)); err != nil {
			return err
		}
	}
	...
	// Ensure the PrefixSet is realized on the provider.
	err := s.Provider.EnsurePrefixSet(ctx, &provider.PrefixSetRequest{
		PrefixSet:      s.PrefixSet,
		ProviderConfig: s.ProviderConfig,
	})

	cond := conditions.FromError(err)
	conditions.Set(s.PrefixSet, cond)

	return err
}

func (r *PrefixSetReconciler) finalize(ctx context.Context, s *prefixSetScope) (reterr error) {
	if err := s.Provider.Connect(ctx, s.Connection); err != nil {
		return fmt.Errorf("failed to connect to provider: %w", err)
	}
	defer func() {
	...
	return s.Provider.DeletePrefixSet(ctx, &provider.PrefixSetRequest{
		PrefixSet:      s.PrefixSet,
		ProviderConfig: s.ProviderConfig,
	})
}
...
func (r *PrefixSetReconciler) deviceToPrefixSets(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	for _, i := range list.Items {
		log.V(2).Info("Enqueuing PrefixSet for reconciliation", "PrefixSet", klog.KObj(&i))
...
// prefixSetsForProviderConfig is a [handler.MapFunc] to be used to enqueue requests for reconciliation
// for a PrefixSet to update when one of its referenced provider configurations gets updated.
func (r *PrefixSetReconciler) prefixSetsForProviderConfig(ctx context.Context, obj client.Object) []reconcile.Request {
	log := ctrl.LoggerFrom(ctx, "Object", klog.KObj(obj))

	list := &v1alpha1.PrefixSetList{}
	...
	for _, m := range list.Items {
		if m.Spec.ProviderConfigRef != nil &&
			...
			log.V(2).Info("Enqueuing PrefixSet for reconciliation", "PrefixSet", klog.KObj(&m))
