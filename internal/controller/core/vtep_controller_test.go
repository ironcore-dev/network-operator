...
var _ = Describe("VTEP Controller", func() {
	...
	Context("When using a non registered dependency for providerConfigRef", func() {
		const name = "test-vtep-misconfigured-providerconfigref"
		key := client.ObjectKey{Name: name, Namespace: metav1.NamespaceDefault}
		...
		BeforeEach(func() {
			...
			vtep = &v1alpha1.VTEP{
				...
				Spec: v1alpha1.VTEPSpec{
					...
					SuppressARP:         true,
					HostReachability:    "BGP",
					PrimaryInterfaceRef: v1alpha1.LocalObjectReference{Name: "lo6"},
					AnycastInterfaceRef: v1alpha1.LocalObjectReference{Name: "lo7"},
					Enabled:             true,
