...
func (r *DNSReconciler) Reconcile(ctx context.Context, req ctrl.Request) (_ ctrl.Result, reterr error) {
	...
	prov, err := provider.LoadProvider[provider.DNSProvider](device.Spec.Provider)
	if err != nil {
...
func (r *DNSReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager) error {
	...
	for _, gvk := range v1alpha1.DNSDependencies {
		obj := &unstructured.Unstructured{}
		obj.SetGroupVersionKind(gvk)

		bldr = bldr.Watches(
			obj,
			handler.EnqueueRequestsFromMapFunc(r.dnsForProviderConfig),
...
func (r *DNSReconciler) deviceToDNSs(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	for _, i := range list.Items {
		log.V(2).Info("Enqueuing DNS for reconciliation", "DNS", klog.KObj(&i))
...
// dnsForProviderConfig is a [handler.MapFunc] to be used to enqueue requests for reconciliation
// for a DNS to update when one of its referenced provider configurations gets updated.
func (r *DNSReconciler) dnsForProviderConfig(ctx context.Context, obj client.Object) []reconcile.Request {
	log := ctrl.LoggerFrom(ctx, "Object", klog.KObj(obj))

	list := &v1alpha1.DNSList{}
	...
	for _, m := range list.Items {
		if m.Spec.ProviderConfigRef != nil &&
			...
			log.V(2).Info("Enqueuing DNS for reconciliation", "DNS", klog.KObj(&m))
