...
func (r *AccessControlListReconciler) Reconcile(ctx context.Context, req ctrl.Request) (_ ctrl.Result, reterr error) {
	...
	prov, err := provider.LoadProvider[provider.ACLProvider](device.Spec.Provider)
	if err != nil {
	...
	defer func() {
		if err := r.Locker.ReleaseLock(ctx, device.Name, "acl-controller"); err != nil {
...
func (r *AccessControlListReconciler) deviceToAccessControlLists(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	for _, i := range list.Items {
		log.V(2).Info("Enqueuing AccessControlList for reconciliation", "AccessControlList", klog.KObj(&i))
...
func (r *AccessControlListReconciler) accessControlListsForProviderConfig(ctx context.Context, obj client.Object) []reconcile.Request {
	...
	for _, m := range list.Items {
		if m.Spec.ProviderConfigRef != nil &&
			...
			log.V(2).Info("Enqueuing AccessControlList for reconciliation", "AccessControlList", klog.KObj(&m))
