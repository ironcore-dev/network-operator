...
func (r *EVPNInstanceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (_ ctrl.Result, reterr error) {
	...
	prov, err := provider.LoadProvider[provider.EVPNInstanceProvider](device.Spec.Provider)
	if err != nil {
	...
	defer func() {
		if err := r.Locker.ReleaseLock(ctx, device.Name, "evpn-instance-controller"); err != nil {
