...
func (r *SNMPReconciler) Reconcile(ctx context.Context, req ctrl.Request) (_ ctrl.Result, reterr error) {
	...
	prov, err := provider.LoadProvider[provider.SNMPProvider](device.Spec.Provider)
	if err != nil {
	...
	defer func() {
		if err := r.Locker.ReleaseLock(ctx, device.Name, "snmp-controller"); err != nil {
...
func (r *SNMPReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager) error {
	...
	for _, gvk := range v1alpha1.SNMPDependencies {
		obj := &unstructured.Unstructured{}
		obj.SetGroupVersionKind(gvk)

		bldr = bldr.Watches(
			obj,
			handler.EnqueueRequestsFromMapFunc(r.snmpForProviderConfig),
...
func (r *SNMPReconciler) deviceToSNMPs(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	for _, i := range list.Items {
		log.V(2).Info("Enqueuing SNMP for reconciliation", "SNMP", klog.KObj(&i))
...
// for a SNMP to update when one of its referenced provider configurations gets updated.
func (r *SNMPReconciler) snmpForProviderConfig(ctx context.Context, obj client.Object) []reconcile.Request {
	log := ctrl.LoggerFrom(ctx, "Object", klog.KObj(obj))

	list := &v1alpha1.SNMPList{}
	...
	for _, m := range list.Items {
		if m.Spec.ProviderConfigRef != nil &&
			...
			log.V(2).Info("Enqueuing SNMP for reconciliation", "SNMP", klog.KObj(&m))
