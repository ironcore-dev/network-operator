...
func TestHandleStatusReport(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			body:           StatusReport{Serial: "ABC123", Status: v1alpha1.ProvisioningScriptExecutionStarted},
		...
		},
		{
			...
			method:         http.MethodPut,
			authorization:  "Bearer validtoken",
			body:           StatusReport{Serial: "NONEXISTENT", Status: v1alpha1.ProvisioningScriptExecutionStarted},
		...
		},
		{
			...
			body:          StatusReport{Serial: "ABC123", Status: v1alpha1.ProvisioningDownloadingImage},
			device: &v1alpha1.Device{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test-device",
					Namespace: "default",
		...
		},
		{
			...
			method:        http.MethodPut,
			authorization: "Bearer validtoken",
			body:          StatusReport{Serial: "ABC123", Status: v1alpha1.ProvisioningRebootingDevice, Detail: "Device is rebooting"},
			device: &v1alpha1.Device{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test-device",
					Namespace: "default",
			...
			validateDevice: func(t *testing.T, device *v1alpha1.Device) {
				assert.Equal(t, v1alpha1.DevicePhaseProvisioningCompleted, device.Status.Phase)
		...
		},
		{
			...
			method:        http.MethodPut,
			authorization: "Bearer validtoken",
			body:          StatusReport{Serial: "ABC123", Status: v1alpha1.ProvisioningScriptExecutionFailed, Detail: "Script execution failed"},
			device: &v1alpha1.Device{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test-device",
					Namespace: "default",
	...
	tests := []struct {
		...
		},
		{
			...
			device: &v1alpha1.Device{
				...
				Status: v1alpha1.DeviceStatus{SerialNumber: "ABC123", Phase: v1alpha1.DevicePhaseActive},
			},
			secret: &corev1.Secret{
				Type: corev1.SecretTypeBasicAuth,
				ObjectMeta: metav1.ObjectMeta{
...
func TestGetMTLSClientCA(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expectedStatus: http.StatusNotFound,
			expectedBody:   "Device has no MTLS configuration",
		},
		{
			...
			expectedStatus: http.StatusInternalServerError,
			expectedBody:   "Failed to get CA certificate",
		},
		{
