...
func (s *HTTPServer) findDeviceAndValidateToken(ctx context.Context, serial, token string) (*v1alpha1.Device, *v1alpha1.ProvisioningInfo, int, error) {
	...
	if act == nil {
		...
		return nil, nil, http.StatusNotFound, fmt.Errorf("no active provisioning found for device: %s", device.Name)
	}
	if act.Token != token {
		return nil, nil, http.StatusUnauthorized, fmt.Errorf("Unauthorized: Invalid token")
	}
...
type StatusReport struct {
	...
	Status v1alpha1.ProvisioningReasonType `json:"status"`
...
func (s *HTTPServer) HandleStatusReport(w http.ResponseWriter, r *http.Request) {
	...
	if report.Succeeded() {
		device.Status.Phase = v1alpha1.DevicePhaseProvisioningCompleted
	...
	s.Recorder.Eventf(device, eventtype, "Provisioning", "%s: %s", report.Status, report.Detail)
...
func (s *HTTPServer) HandleProvisioningRequest(w http.ResponseWriter, r *http.Request) {
	...
	response := ProvisioningResponse{
		...
		Hostname:          device.ObjectMeta.Name,
	}
...
func (s *HTTPServer) GetMTLSClientCA(w http.ResponseWriter, r *http.Request) {
	...
	if device.Spec.Endpoint.TLS == nil {
		http.Error(w, "Device has no MTLS configuration", http.StatusNotFound)
		return
	}

	operatorCA, err := c.Secret(ctx, &device.Spec.Endpoint.TLS.CA)
	if err != nil {
		s.Logger.Error(err, "Failed to get CA certificate", "device", device.Name)
...
func (s *HTTPServer) GetDeviceCertificate(w http.ResponseWriter, r *http.Request) {
	...
	if certificate, ok := certSecret.Data["tls.crt"]; ok {
		response.Certificate = certificate
	}
	if privateKey, ok := certSecret.Data["tls.key"]; ok {
		response.PrivateKey = privateKey
	}
	if caCertificate, ok := certSecret.Data["ca.crt"]; ok {
