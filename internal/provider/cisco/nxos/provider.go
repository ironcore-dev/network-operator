...
func (p *Provider) EnsureInterface(ctx context.Context, req *provider.InterfaceRequest) (provider.Result, error) {
	...
	case v1alpha1.InterfaceTypePhysical:
		...
		if len(req.Interface.Spec.IPv4Addresses) > 0 {
			...
			addr, ok := strings.CutPrefix("unnumbered:", req.Interface.Spec.IPv4Addresses[0])
			switch {
			case ok:
				l3opts = append(l3opts, iface.WithUnnumberedAddressing(addr)) // Extract the source interface name
			default:
