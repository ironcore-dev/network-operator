...
var _ = Describe("NVE Webhook", func() {
	var (
		obj       *corev1alpha1.NVE
		oldObj    *corev1alpha1.NVE
		validator NVECustomValidator
	)

	BeforeEach(func() {
		obj = &corev1alpha1.NVE{
			Spec: corev1alpha1.NVESpec{
	...
	Context("ValidateDelete", func() {
		...
		It("rejects delete when object type is wrong", func() {
			_, err := validator.ValidateDelete(ctx, &corev1alpha1.NVEList{})
			Expect(err).To(HaveOccurred())
		})
	})
})
