...
var _ = Describe("NVEConfig Webhook", func() {
	var (
		obj       *nxv1alpha1.NVEConfig
		oldObj    *nxv1alpha1.NVEConfig
		validator NVEConfigCustomValidator
	)
	...
	Context("ValidateDelete", func() {
		It("allows delete on NVEConfig object", func() {
			_, err := validator.ValidateDelete(ctx, obj)
			Expect(err).NotTo(HaveOccurred())
		})

		It("rejects delete when object type is wrong", func() {
			_, err := validator.ValidateDelete(ctx, &nxv1alpha1.NVEConfigList{})
			Expect(err).To(HaveOccurred())
		})
	})
})
