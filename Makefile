...
.PHONY: fmt
fmt: goimports gofumpt ## Run goimports against code.
	$(GOIMPORTS) -w .
	$(GOFUMPT) -l -w $(shell git ls-files '*.go' | grep -E -v 'internal/provider/openconfig|internal/provider/cisco/nxos/genyang')
...
check-license: addlicense ## Check that every file has a license header present.
	find . -name '*.go' -exec $(ADDLICENSE) -check -c 'IronCore contributors' {} +
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name foo-builder
	$(CONTAINER_TOOL) buildx use foo-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm foo-builder
	rm Dockerfile.crosss
...
.PHONY: kind-create
kind-create: $(ENVTEST) ## create kind cluster if needed
	./hack/kind-with-registry.sh

.PHONY: kind-delete
kind-delete: ## Destroys the kind cluster.
