...
func (c *ClusterEnvironment) Teardown(ctx context.Context) error {
	_ = c.runKubectl(ctx, "delete", "-k", "config/crd", "--ignore-not-found")
	return nil
}
...
func (c *ClusterEnvironment) DeployGNMIServer(ctx context.Context, namespace string) (netip.AddrPort, error) {
	...
	out, err := c.runKubectlOutput(ctx,
...
func (c *ClusterEnvironment) WaitForTestNamespacesGone(ctx context.Context) error {
	...
	for _, ns := range namespaces {
		_ = c.runKubectl(ctx, "wait", "namespace", ns,
			"--for=delete",
			"--timeout=120s")
	}
...
// DeleteCustomResources deletes all resources of the CRD kinds defined in ResourceRegistry in the given namespace.
// Resources are deleted in reverse order so that dependents (e.g., BGP) are deleted before their
// config resources (e.g., BGPConfig), allowing finalizers to complete successfully.
// Device is deleted last after all other resources are gone.
func (c *ClusterEnvironment) DeleteCustomResources(ctx context.Context, namespace string) error {
	// Delete in reverse order: dependents before their dependencies
	for i := len(ResourceRegistry) - 1; i >= 0; i-- {
		gvk := ResourceRegistry[i]
		_ = c.runKubectl(ctx, "delete",
			ResourcePluralName(gvk), "--all",
			"--namespace", namespace,
			"--ignore-not-found")
	}

	// Wait for all resources to be fully gone (finalizers completed)
	for i := len(ResourceRegistry) - 1; i >= 0; i-- {
		gvk := ResourceRegistry[i]
		_ = c.runKubectl(ctx, "wait",
	...
	_ = c.runKubectl(ctx, "delete", "devices", "--all",
		"--namespace", namespace,
		"--ignore-not-found")
	_ = c.runKubectl(ctx, "wait", "devices",
		"--for=delete", "--all",
		"--namespace", namespace,
		"--timeout=60s")
