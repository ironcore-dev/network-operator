...
// Resources are deleted in reverse order (last in array = deleted first).
// Config resources must be at the START so they're deleted LAST.
// Device is NOT included - it must remain until all finalizers complete.
var ResourceRegistry = []schema.GroupVersionKind{
	// Config resources - deleted after main resources
	...
	// Main resources - deleted FIRST (have finalizers that need Device and configs)
